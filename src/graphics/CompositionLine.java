...
public class CompositionLine extends ConnectionLine
{
    ...
    private final GeneralPath arrowhead = new GeneralPath(GeneralPath.WIND_NON_ZERO, 4);

    public CompositionLine(BasicObject startBasicObject, BasicObject.ConnectionPort startPort, BasicObject endBasicObject, BasicObject.ConnectionPort endPort)
    {
    ...
    public void draw(Graphics g)
    {
        ...
        // draw a horizontal line from (0, 0) to (length - 16 * sqrt(3), 0) with an arrowhead that forms a diamond shape with angles of 60 degrees and 120 degrees
        arrowShaft.setLine(0, 0, length - 16 * Math.sqrt(3), 0);
        arrowhead.reset();
        arrowhead.moveTo(length - 16 * Math.sqrt(3), 0);
